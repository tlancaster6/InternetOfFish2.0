...
class Uploader:
    ...
    def __init__(self, local_project_dir, cloud_data_dir=None, video_framerate=30):
        logger.info('Beginning Uploader initialization')
        self.local_project_dir = Path(local_project_dir)
        if cloud_data_dir is None:
            self.attempt_uploads = False
            logger.info('cloud_data_dir was "None", so uploads will not be attempted')
        else:
            ...
            logger.info(f'uploads will be sent to {self.cloud_project_dir}')
        self.framerate = video_framerate
        logger.debug('Uploader successfully initialized\n\n')
    ...
    def convert_h264s_to_mp4s(self):
        logger.info('converting h264s for mp4s')
